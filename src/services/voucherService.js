...
//==================STRATEGY VOUCHER====================//
let applyVoucher = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.voucherId || !data.amount) {
                resolve({
                    errCode: 1,
            ...
            const voucherContext = new VoucherContext();
            const result = await voucherContext.calculateDiscount(data.voucherId, data.amount);

            if (result.errCode === 0 && data.userId) {
